...
export async function getAllExamIds(): Promise<string[] | null> {
    console.log('Attempting to retrieve all examIds');

    // Try to get the examIds from the cache
    let examIds = await safeKVOperation(() => kv.get<string[]>(EXAM_IDS_CACHE_KEY));
    ...
    if (!examIds) {
        ...
        if (examIds.length > 0) {
            console.log('Storing examIds in cache:', examIds);
            await safeKVOperation(() => kv.set(EXAM_IDS_CACHE_KEY, examIds));
        }
    }

    console.log('Retrieved examIds:', examIds);
    return examIds;
}
